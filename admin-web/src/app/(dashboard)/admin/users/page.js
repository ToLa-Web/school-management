...
import {
  ...
  ShieldCheck, GraduationCap, BookOpen, User, ChevronDown, Check, Loader2, Search, X as XIcon,
} from 'lucide-react';
...
export default function UsersPage() {
  ...
  function getFilteredUsers() {
    return users.filter((u) => {
      ...
      const roleLabel = roleInfo(u.role ?? u.userRole).label.toLowerCase();
      const verified = u.isEmailVerified ? 'verified' : 'pending';

      const nameMatch = fullName.includes(filterName.toLowerCase());
      const emailMatch = email.includes(filterEmail.toLowerCase());
      const roleMatch = !filterRole || roleLabel === filterRole.toLowerCase();
      const verifiedMatch = !filterVerified || verified === filterVerified.toLowerCase();
  ...
  const hasFilters = filterName || filterEmail || filterRole || filterVerified;
    ...
    <div className="admin-page">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              {hasFilters 
                ...
                : `${users.length} accounts in auth service`}
            </p>
          </div>
        </div>
        <button
      ...
      <div className="bg-slate-50 rounded-xl border border-slate-100 p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          {/* Role filter */}
          <div>
            <label className="text-xs font-semibold text-slate-700 mb-1.5 block">Filter by Role</label>
            <select
              value={filterRole}
          ...
          {/* Verified filter */}
          <div>
            <label className="text-xs font-semibold text-slate-700 mb-1.5 block">Filter by Status</label>
            <select
        ...
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-slate-500 text-sm">
              {hasFilters ? 'No accounts match your filters.' : 'No accounts found.'}
            </p>
          </div>
        ) : (
              ...
              {filteredUsers.map((u, i) => (
                <tr key={u.id ?? i} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-[#526d82] flex items-center justify-center text-white font-semibold text-xs shrink-0">
                  ...
                  <td className="px-5 py-3.5 text-right">
                    <button
                      onClick={() => handleDeleteClick(u.id, `${u.firstName} ${u.lastName}`)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-[#526d82] flex items-center justify-center">
            ...
            <form onSubmit={handleCreate} className="p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-1">
                <button type="submit" disabled={saving}
                  className="flex-1 bg-[#526d82] hover:bg-[#27374d] disabled:opacity-50 text-white text-sm font-medium px-5 py-2.5 rounded-xl shadow-lg shadow-slate-500/20 transition-all flex items-center justify-center gap-2">
      ...
      <DeleteConfirmDialog
        ...
        message={`Are you sure you want to delete the login account for ${deleteTarget?.name}? They will no longer be able to log in. This action cannot be undone.`}
